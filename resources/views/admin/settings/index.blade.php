            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2 border-t pt-4 mt-2">
                    <h4 class="text-md font-medium text-red-600 mb-3 flex items-center">
                        ...
                        {{ __('admin.maintenance_mode') ?? 'Maintenance Mode' }}
                    </h4>

                    <div class="flex items-center gap-4 mb-4">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="maintenance_mode" value="1" {{ (old('maintenance_mode', $settings['maintenance_mode'] ?? '0') == '1') ? 'checked' : '' }} class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-red-600"></div>
                            <span class="ml-3 text-sm font-medium text-gray-700">{{ __('admin.enable_maintenance_mode') ?? 'Enable Maintenance Mode' }}</span>
                        </label>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.maintenance_message') ?? 'Maintenance Message' }}</label>
                        <textarea name="maintenance_message" rows="2" placeholder="{{ __('admin.maintenance_placeholder') ?? 'We are currently performing maintenance. Please check back soon.' }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('maintenance_message', $settings['maintenance_message'] ?? '') }}</textarea>
                    </div>
                </div>
