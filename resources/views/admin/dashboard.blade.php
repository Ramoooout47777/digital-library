...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-5 hover:shadow-md transition duration-200 border-l-4 border-yellow-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-500">{{ __('admin.total_revenue') }} - 2026</p>
                <p class="text-2xl font-bold text-gray-800">${{ number_format($stats['revenue_2026'] ?? 0, 2) }}</p>
                <p class="text-xs text-green-500 mt-1">
                    <i class="fas fa-chart-line"></i>
                    {{ __('admin.total') }}: ${{ number_format($stats['total_revenue'], 2) }}
                </p>
            </div>
